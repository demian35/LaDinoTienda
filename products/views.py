...
from clients.models import ConvenienceStore
...
def productDetail(request, id):
    if request.user.is_authenticated:   
        ...
        store = ConvenienceStore.objects.get(user_id=user_id)
        if request.method == "POST":
            form = cartForm(request.POST)
            
            if form.is_valid():
                quantity = form.cleaned_data['quantity']
                latest_cart = Cart.objects.latest('id')
                cart = form.save(commit=False)
                cart.id=latest_cart.id+1
                cart.bought = False
                cart.id_convenience_store= store
                cart.id_producto = producto 
            ...
            form = cartForm()
        return render(request, "detalleProducto.html", {"product": producto, "form": form, "username": user})    
    else:
        return redirect('Dino iniciar sesion')
